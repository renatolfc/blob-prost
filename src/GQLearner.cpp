/****************************************************************************************
 ** Implementation of GQ(lambda). It implements Fig. 8.8 (Linear, gradient-descent
 ** GQ(lambda)) from the book "R. Sutton and A. Barto; Reinforcement Learning: An
...
GQLearner::GQLearner(ALEInterface& ale, Features *features, Parameters *param,int seed) : RLLearner(ale, param,seed) {
...
void GQLearner::updateQValues(vector<long long> &Features, vector<float> &QValues){
    unsigned long long featureSize = Features.size();
    for(int a = 0; a < numActions; ++a){
        float sumW = 0;
        for(unsigned long long i = 0; i < featureSize; ++i){
            sumW = sumW + theta[a][Features[i]]*groups[Features[i]].numFeatures;
        }
        QValues[a] = sumW;
    }
}

void GQLearner::updateReplTrace(int action, vector<long long> &Features){
    //e <- gamma * lambda * e
...
void GQLearner::updateAcumTrace(int action, vector<long long> &Features){
    //e <- gamma * lambda * e
    for(unsigned int a = 0; a < nonZeroElig.size(); a++){
        long long numNonZero = 0;
...
void GQLearner::saveCheckPoint(int episode, int totalNumberFrames, vector<float>& episodeResults,int& frequency,vector<int>& episodeFrames, vector<double>& episodeFps){
    ofstream learningConditionFile;
    ...
    for (unsigned long long groupIndex=0; groupIndex<numGroups;++groupIndex){
        ...
        for (unsigned long long a=0; a<theta.size();a++){
            if (theta[a][groupIndex]!=0){
        ...
        for (int i=0;i<nonZeroWeights.size();++i){
            ...
            checkPointFile<<" "<<action<<" "<<theta[action][groupIndex];
        }
        checkPointFile<<"\t";
    }
    checkPointFile << endl;
...
void GQLearner::loadCheckPoint(ifstream& checkPointToLoad){
    ...
    for (unsigned a =0;a<theta.size();a++){
        theta[a].resize(numGroups,0.00);
    ...
    for (unsigned long long groupIndex=0; groupIndex<numGroups;++groupIndex){
        ...
        for (unsigned int i=0; i<numNonZeroWeights;++i){
            ...
            theta[action][groupIndex] = weight;
        }
    }
...
void GQLearner::learnPolicy(ALEInterface& ale, Features *features){
    ...
    for(int episode = episodePassed+1; totalNumberFrames < totalNumberOfFramesToLearn; episode++){
        ...
        while(!ale.game_over()){
            ...
            greedyAction = Mathematics::argmax(Q, agentRand);
            if (currentAction == greedyAction) {
                pi = 1 - epsilon;
                rho = 1.0;
            } else {
                pi = epsilon / numActions;
                if (fakeAle) {
                    rho = pi;
                } else {
                    // b = epsilon / numAction. Hence:
                    rho = 1;
                }
            }
            ...
            for(unsigned int a = 0; a < nonZeroElig.size(); a++){
                for(unsigned int i = 0; i < nonZeroElig[a].size(); i++){
                    long long idx = nonZeroElig[a][i];
                    theta[a][idx] = theta[a][idx] + learningRate * delta * e[a][idx];
                }
            }
            F = Fnext;
...
void GQLearner::evaluatePolicy(ALEInterface& ale, Features *features){
    double reward = 0;
    double cumReward = 0;
...
void GQLearner::groupFeatures(vector<long long>& activeFeatures){
    ...
    for (unsigned long long i = 0; i <activeFeatures.size();++i){
        ...
        if (featureTranslate[featureIndex] == 0){
            if (newGroup){
                ...
                agroup.features.clear();
                groups.push_back(agroup);
                for (unsigned int action=0;action<theta.size();++action){
                    theta[action].push_back(0.0);
    ...
    for (unsigned long long index=0;index<activeGroupIndices.size();++index){
        ...
        if (groups[groupIndex].features.size()!=groups[groupIndex].numFeatures && groups[groupIndex].features.size()!=0){
            ...
            for (unsigned a = 0;a<theta.size();++a){
                theta[a].push_back(theta[a][groupIndex]);
...
void GQLearner::saveWeightsToFile(string suffix){
    std::ofstream weightsFile ((nameWeightsFile + suffix).c_str());
    if(weightsFile.is_open()){
        weightsFile << theta.size() << " " << theta[0].size() << std::endl;
        for(unsigned int i = 0; i < theta.size(); i++){
            for(unsigned int j = 0; j < theta[i].size(); j++){
                if(theta[i][j] != 0){
                    weightsFile << i << " " << j << " " << theta[i][j] << std::endl;
                }
            }
        }
        weightsFile.close();
