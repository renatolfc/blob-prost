/****************************************************************************************
 ** Implementation of GQ(lambda).
 ***************************************************************************************/

// We assume that the behavior policy will *always* give probability one to the
// action being taken. Hence: \rho_t = \frac{\pi(S_t, A_t)}{b(S_t, A_t)}
// = \pi(S_t, A_t).
...
class GQLearner : public RLLearner{
private:
    ...
    vector<vector<float>> theta;            //Theta, weights vector
    ...
    /**
     * Constructor declared as private to force the user to instantiate GQLearner
    ...
    void sanityCheck();
    /**
     * In GQ the Q-values (one per action) are updated as the sum of weights for that given action.
...
public:
    GQLearner(ALEInterface& ale, Features *features, Parameters *param,int seed);
    /**
     * Implementation of an agent controller. This implementation is GQ(lambda).
     *
